...
_find_steam_libraries() {
	...
	grep -E '"path"' "$vdf" | sed -E 's/.*"path"[[:space:]]+"(.*)"/\1/' | sort -u
}
...
_check_dependencies() {
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mDependency Check\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
...
_check_radio_connection() {
	...
	# -i: pressure-vessel shows "Arma3_x64.exe" (capital A), case varies
	if ! pgrep -if "Arma3_x64" >/dev/null 2>&1; then
...
# _mod_meta_from_workshop
#   Extract mod metadata from mod.cpp inside the Workshop mod directory.
#   Handles: tabs between key and =, inline // comments, Windows \r,
#   quoted values with semicolons, missing fields.
#   Output: "name|author" (pipe-separated, empty if field absent).
_mod_meta_from_workshop() {
	local id="$1" lib_path
	while IFS= read -r lib_path; do
		[[ -z "$lib_path" ]] && continue
		local moddir="$lib_path/steamapps/workshop/content/107410/$id"
		[[ -d "$moddir" ]] || continue
		local mc
		mc="$(find "$moddir" -iname "mod.cpp" -print -quit 2>/dev/null)"
		[[ -n "$mc" && -f "$mc" ]] || continue

		# Extract a quoted value for a given key from mod.cpp.
		# Strips: CR, leading tabs/spaces, inline // comments, trailing semicolons.
		_extract() {
			local key="$1" raw=""
			raw="$(sed -n "s/^${key}[[:space:]]*=[[:space:]]*\"\([^\"]*\)\".*/\1/p" "$mc" | head -1)"
			raw="${raw%%$'\r'}"                  # strip CR
...
_mod_name_from_workshop() {
	local meta
	meta="$(_mod_meta_from_workshop "$1")" || return 1
	echo "${meta%%|*}"
}

# _workshop_size_human
#   Convert bytes to a compact human-readable size (e.g., 668M, 13G).
_workshop_size_human() {
	local bytes="$1"
...
# _parse_workshop_vdf
#   Parse the Workshop VDF (appworkshop_107410.acf) and populate
#   associative arrays: _WV_SIZE[id] = bytes, _WV_TIME[id] = unix ts.
#   Searches all Steam libraries and merges entries (largest VDF wins).
_parse_workshop_vdf() {
	declare -gA _WV_SIZE=() _WV_TIME=()
	local vdf
	while IFS= read -r lib_path; do
		[[ -z "$lib_path" ]] && continue
		vdf="$lib_path/steamapps/workshop/appworkshop_107410.acf"
		[[ -f "$vdf" ]] || continue

		local cur_id="" in_items=0
		while IFS= read -r line; do
			[[ "$line" == *"WorkshopItemsInstalled"* ]] && in_items=1
			((in_items)) || continue
			if [[ "$line" =~ ^[[:space:]]*\"([0-9]+)\"[[:space:]]*$ ]]; then
				cur_id="${BASH_REMATCH[1]}"
				continue
			fi
			if [[ -n "$cur_id" && "$line" =~ \"size\"[[:space:]]+\"([0-9]+)\" ]]; then
				_WV_SIZE["$cur_id"]="${BASH_REMATCH[1]}"
				continue
			fi
			if [[ -n "$cur_id" && "$line" =~ \"timeupdated\"[[:space:]]+\"([0-9]+)\" ]]; then
				_WV_TIME["$cur_id"]="${BASH_REMATCH[1]}"
				cur_id=""
				continue
			fi
		done <"$vdf"
...
#   libraries, with human-readable names from mod.cpp and sizes from
#   the Workshop VDF.
_list_installed_mods() {
	_parse_workshop_vdf

	# Pass 1: collect all mod data.
	local -a ids=() names=() authors=() sizes=()
	local -A seen_names=()
	local max_name=4 max_author=6 max_size=4 found=0 noname_count=0 total_bytes=0
	local lib_path ws
	while IFS= read -r lib_path; do
		[[ -z "$lib_path" ]] && continue
		while IFS= read -r ws; do
			...
			[[ "$id" == "107410" ]] && continue
			local meta size_str
			meta="$(_mod_meta_from_workshop "$id")"
			local name author
			if [[ -n "$meta" ]]; then
				name="${meta%%|*}"
				author="${meta#*|}"
			fi
			size_str="$(_workshop_size_human "${_WV_SIZE[$id]:-}")"
			if [[ -n "$name" ]]; then
				# Skip duplicate names (multiple Workshop IDs for the same mod)
				if [[ -n "${seen_names[$name]+_}" ]]; then
					continue
				fi
				seen_names["$name"]=1
				[[ -z "$size_str" ]] && size_str="-"
				ids+=("$id") names+=("$name") authors+=("$author") sizes+=("$size_str")
				total_bytes=$((total_bytes + ${_WV_SIZE[$id]:-0}))
				((${#name} > max_name)) && max_name=${#name}
				((${#author} > max_author)) && max_author=${#author}
				((${#size_str} > max_size)) && max_size=${#size_str}
			else
				noname_count=$((noname_count + 1))
			fi
			found=1
	...
	# Pass 2: print table with borders.
	local sep_w=$((max_name + max_author + max_size + 6))
	local sep
	sep="$(printf '%*s' "$sep_w" '' | tr ' ' '-')"
	local _bld _cyn _grn _yel _red _rst _dim
	_bld='\e[1m' _cyn='\e[36m' _grn='\e[32m' _yel='\e[33m' _red='\e[31m' _rst='\e[0m' _dim='\e[2m'
	echo -e "  ${_cyn}┌${_bld}──────────────${_rst}${_cyn}┬${sep}┐${_rst}"
	printf "  ${_cyn}│${_bld} %-12s${_rst}${_cyn} │${_bld} %-${max_name}s${_rst}  ${_cyn}%-${max_author}s${_rst}  %-${max_size}s ${_cyn}│${_rst}\n" \
		"Workshop ID" "Name" "Author" "Size"
	echo -e "  ${_cyn}├${_bld}──────────────${_rst}${_cyn}┼${sep}┤${_rst}"
	local i=0 count=${#ids[@]} _sz_val
	for ((i = 0; i < count; i++)); do
		_sz_val="${sizes[$i]}"
		local _scol="$_rst" _nrow="$_rst"
		[[ "${names[$i]}" == "(unknown)" ]] && _nrow="$_dim"
		case "$_sz_val" in
		*K | *M)
			[[ "${_sz_val%[KM]}" -ge 100 && "$_sz_val" == *M ]] && _scol="$_yel" || _scol="$_grn"
			;;
		*G) _scol="$_red" ;;
		esac
		printf "  ${_cyn}│${_nrow} %-12s${_cyn} │${_nrow} %-${max_name}s  %-${max_author}s  ${_scol}%- ${max_size}s${_rst}${_cyn} │${_rst}\n" \
			"${ids[$i]}" "${names[$i]}" "${authors[$i]}" "${sizes[$i]}"
	done
	echo -e "  ${_cyn}└${_bld}──────────────${_rst}${_cyn}┴${sep}┘${_rst}"
	local _total_human
	_total_human="$(_workshop_size_human "$total_bytes")"
	echo -e "  ${_bld}${count} mods${_rst} installed, ${_cyn}${_total_human}${_rst} total"
	echo -e "  ${_grn}■${_rst} <100M  ${_yel}■${_rst} 100M–1G  ${_red}■${_rst} >1G"
	if [[ "$noname_count" -gt 0 ]]; then
		echo ""
		echo -e "  ${_dim}$noname_count additional mod(s) with no mod.cpp${_rst}"
	fi
}
...
_list_loaded_mods() {
	...
	# Pass 1: collect data, find max widths.
	local -a ids=() names=() authors=() sizes=()
	local max_name=4 max_author=6 max_size=4 count=0 acre2=0 tfar=0
	while IFS= read -r id; do
		[[ -z "$id" ]] && continue
		local meta size_str
		meta="$(_mod_meta_from_workshop "$id")"
	...
	# Pass 2: print table with borders.
	# Total width = 2(spaces) + 1(│) + 1(space) + 12(ID) + 1(space) + 1(│) + 1(space)
	#              + max_name + 2(spaces) + max_author + 2(spaces) + max_size + 1(space) + 1(│)
	#            = max_name + max_author + max_size + 25
	# Separator = 2(spaces) + 1(├) + 14(dashes) + 1(┼) + sep + 1(┤) = 19 + sep
	local sep_w=$((max_name + max_author + max_size + 6))
	local sep
	sep="$(printf '%*s' "$sep_w" '' | tr ' ' '-')"
	# ponytail: colour is plain ANSI, no tput dependency; drop if $TERM is dumb
	local _bld _cyn _grn _yel _red _rst _dim
	_bld='\e[1m' _cyn='\e[36m' _grn='\e[32m' _yel='\e[33m' _red='\e[31m' _rst='\e[0m' _dim='\e[2m'
	echo -e "  ${_cyn}┌${_bld}──────────────${_rst}${_cyn}┬${sep}┐${_rst}"
	printf "  ${_cyn}│${_bld} %-12s${_rst}${_cyn} │${_bld} %-${max_name}s${_rst}  ${_cyn}%-${max_author}s${_rst}  %-${max_size}s ${_cyn}│${_rst}\n" \
	...
	local i=0 _sz_val
	for ((i = 0; i < count; i++)); do
		# Colour the size column: green <100M, yellow <1G, red ≥1G
		_sz_val="${sizes[$i]}"
		local _scol="$_rst" _nrow="$_rst"
		[[ "${names[$i]}" == "(unknown)" ]] && _nrow="$_dim"
		case "$_sz_val" in
	...
	echo -e "  ${_cyn}└${_bld}──────────────${_rst}${_cyn}┴${sep}┘${_rst}"
	echo ""
	echo -e "  ${_bld}${count} mods${_rst} loaded in session"
	if [[ "$acre2" == 1 ]]; then
		echo -e "  ${_grn}[LOADED]${_rst}   ACRE2"
	else
		echo -e "  ${_yel}[--  --]${_rst}   ACRE2"
	fi
	if [[ "$tfar" == 1 ]]; then
		echo -e "  ${_grn}[LOADED]${_rst}   TFAR"
	else
		echo -e "  ${_yel}[--  --]${_rst}   TFAR"
	fi
	echo -e "  ${_grn}■${_rst} <100M  ${_yel}■${_rst} 100M–1G  ${_red}■${_rst} >1G"
	return 0
}
	...
	if [[ -n "$_prefix_version" ]]; then
		...
		while IFS= read -r lib_path; do
			...
			if [[ -d "$_sa/common" ]]; then
				...
				done < <(find -L "$_sa/common" -maxdepth 1 -type d -name "Proton*")
			fi
...
_prefix_doctor() {
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mPrefix Doctor – $COMPAT_DATA_PATH\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""

	echo -e "\e[1m--- Marker files ---\e[0m"
	...
	echo ""
	echo -e "\e[1m--- Windows system directory ---\e[0m"
	...
	echo ""
	echo -e "\e[1m--- Drive mappings (dosdevices) ---\e[0m"
	...
	echo ""
	echo -e "\e[1m--- Prefix creation state ---\e[0m"
	...
	echo ""
	echo -e "\e[1m--- Filesystem mount ---\e[0m"
	local mount_point
...
_prefix_reset() {
	...
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	if [[ "$mode" == "full" ]]; then
		echo -e "  \e[1;36mPrefix Reset (full) – recreate prefix\e[0m"
	else
		echo -e "  \e[1;36mPrefix Reset (in-place) – repair Wine system files\e[0m"
	fi
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
	...
	if [[ "$mode" == "full" ]]; then
		...
		echo "  2. Run './Arma3Helper.sh syncprofiles restore' to restore your"
		echo "     profiles from the host copy."
...
_bind_host_dirs() {
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mBind Host Directories\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
	...
	"$PROTONEXEC" run reg add "$key" /v Personal /t REG_EXPAND_SZ /d "$host_docs" /f >/dev/null 2>&1
	"$PROTONEXEC" run reg add "$key" /v Downloads /t REG_EXPAND_SZ /d "$host_dl" /f >/dev/null 2>&1
	"$PROTONEXEC" run reg add "$key" /v "$guid_docs" /t REG_EXPAND_SZ /d "$host_docs" /f >/dev/null 2>&1
	"$PROTONEXEC" run reg add "$key" /v "$guid_dl" /t REG_EXPAND_SZ /d "$host_dl" /f >/dev/null 2>&1
...
_unbind_host_dirs() {
	...
	# Shell Folders is REG_SZ (no variable expansion), so use the literal
	# prefix-local path — not %USERPROFILE%, which Wine stores as-is.
	"$PROTONEXEC" run reg add "$legacy_key" /v Personal /t REG_SZ /d "C:\\users\\steamuser\\Documents" /f >/dev/null 2>&1
	"$PROTONEXEC" run reg add "$legacy_key" /v Downloads /t REG_SZ /d "C:\\users\\steamuser\\Downloads" /f >/dev/null 2>&1
...
# PROFILE SYNC
# -----------------------------------------------------------------------------
# Wine cannot reliably write through the Z: drive into host bind-mounted
# directories under pressure-vessel (Valve steam-runtime issues #369, #429:
# the container's /home is a separate mount and traversal breaks). Arma 3
# therefore saves profiles to the prefix-local Documents, which always
# works. This command keeps the host copy in sync so profiles survive
# prefix deletion or reset.
#
#   syncprofiles backup  - copy prefix -> host (run after an Arma session)
#   syncprofiles restore - copy host -> prefix (run after a fresh prefix)
_sync_profiles() {
	local prefix_docs="$COMPAT_DATA_PATH/pfx/drive_c/users/steamuser/Documents"
	local host_docs="$HOME/Documents"
	local direction="${1:-backup}"

	if [[ ! -d "$prefix_docs" ]]; then
		echo -e "\e[31mError\e[0m: Prefix Documents not found: $prefix_docs"
		return 1
	fi
	...
	*)
		echo "Usage:"
		echo "  ./Arma3Helper.sh syncprofiles backup   Copy prefix profiles to host"
		echo "  ./Arma3Helper.sh syncprofiles restore  Copy host profiles to prefix"
		return 1
		;;
	esac

	echo ""
	echo "Syncing Arma 3 profiles ($label)..."
	echo "  Source: $src"
	echo "  Target: $dst"
	echo ""

	# Sync only the Arma profile trees: the default profile folder and the
	# named-profiles folder. Use rsync when present (preserves mtime and
	# only copies changed files), otherwise fall back to cp.
	# The sync MERGES rather than mirrors: --delete is never used because
	# each side can hold profiles the other does not have, and removing
	# them would lose data.
	local synced=0
	for name in "Arma 3" "Arma 3 - Other Profiles"; do
		if [[ -d "$src/$name" ]]; then
			mkdir -p "$dst/$name"
	...
	if [[ "$synced" == 0 ]]; then
		echo "Nothing to sync (no Arma profile folders found)."
		return 1
	fi

	echo ""
	echo -e "\e[32mDone.\e[0m Profiles synced $label."
	echo "Run this after every Arma session to keep the host copy current."
	return 0
}
...
if [[ -z "$*" ]]; then
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mLaunching TeamSpeak 3 inside Arma 3's Wine prefix\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
...
"install")
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mTeamSpeak 3 Installer\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
...
"listproton")
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mAvailable Proton Versions\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
		...
		if [[ -d "$_sa/common" ]]; then
			while IFS= read -r _dir; do
				if [[ -f "$_dir/proton" ]]; then
					_ver="$(basename "$_dir")"
					if [[ "$_ver" == *"Proton "* ]] && [[ "$_ver" != *"Runtime"* ]] && [[ "$_ver" != *"BattlEye"* ]]; then
						_official_list+="$_ver"$'\n'
					fi
				fi
			done < <(find -L "$_sa/common" -maxdepth 1 -type d -name "Proton*")
...
"debug")
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mDebug Information – Arma3Helper.sh\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
	...
	echo "Proton executable found: $(test -x "$PROTONEXEC" && echo -e '\e[32mYES\e[0m' || echo -e '\e[31mNO\e[0m')"
	echo "TS3 executable found:    $(test -x "$TSPATH" && echo -e '\e[32mYES\e[0m' || echo -e '\e[31mNO\e[0m')"
	echo "Config file:             $USERCONFIG/config"
	echo ""
	echo -e "\e[1m--- Proton Configuration ---\e[0m"
	if [[ -n "$PROTON_CUSTOM_VERSION" ]]; then
		echo "Type:    Custom"
	...
	echo ""
	echo -e "\e[1m--- Environment Variables ---\e[0m"
	...
	echo ""
	echo -e "\e[1m--- Launch Command ---\e[0m"
	echo "\"$PROTONEXEC\" run \"$TSPATH\""
	echo ""
	echo -e "\e[1m--- All Steam Libraries ---\e[0m"
	...
	echo ""
	echo -e "\e[1m--- Custom Proton Builds ---\e[0m"
	_list_custom_proton
	echo ""
	echo "================================================================"
	echo ""
...
"listmods")
	...
	echo ""
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo -e "  \e[1;36mArma 3 Mods\e[0m"
	echo -e "  \e[36m══════════════════════════════════════════════════════════════\e[0m"
	echo ""
	if [[ "$2" == "installed" ]]; then
		echo -e "  \e[1mInstalled Workshop mods\e[0m"
		_list_installed_mods
	elif [[ "$2" == "loaded" ]]; then
		echo -e "  \e[1mMods loaded in latest session\e[0m"
		_list_loaded_mods
	else
		echo -e "  \e[1mMods loaded in latest session\e[0m"
		_list_loaded_mods
...
"syncprofiles")
	# -------------------------------------------------------------------------
	# Keep the host copy of Arma 3 profiles in sync with the prefix-local
	# copy. Wine cannot write through Z: into bind mounts, so Arma saves to
	# the prefix; this mirrors it to the host for durability.
	#
	#   syncprofiles backup  - prefix -> host (after a session)
	#   syncprofiles restore - host -> prefix (after a fresh prefix)
	_sync_profiles "$2"
	;;
