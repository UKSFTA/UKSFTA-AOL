...
Run `./Arma3Helper.sh checkdeps`. It reports both problems automatically.
...
The following parameters help only some systems. Try them individually to identify which help your configuration:

- **`-exThreads=7`**: Offloads work to more threads. Best for CPUs with 4+ cores. Use `3` for 2-core CPUs.
...
Do not use `-malloc` parameters. The old `tbb4malloc_bi` recommendation is outdated and the game allocates memory correctly by default.
