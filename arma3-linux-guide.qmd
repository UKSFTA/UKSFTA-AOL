...
1. Download the TeamSpeak 3 Windows installer from the [official website](https://www.teamspeak.com/en/downloads/#ts3client).

2. Select the **Windows** version. Choose the **64-bit** installer. The filename will resemble `TeamSpeak3-Client-win64-3.x.x.exe`. Save it to a convenient location such as your Downloads folder.
...
After subscribing to TFAR through the Steam Workshop, the mod files will be downloaded to:
...
If the plugin does not appear, check that the files were copied to the correct directory. The plugins folder path is case-sensitive on Linux. Make sure `Program Files` is capitalised correctly.
