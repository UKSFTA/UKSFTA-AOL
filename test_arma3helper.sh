...
eval "$_plugin_fn"
_plugin_fn="$(sed -n '/^_mod_meta_from_workshop() {/,/^}/p' "$HELPER")"
eval "$_plugin_fn"
_plugin_fn="$(sed -n '/^_mod_name_from_workshop() {/,/^}/p' "$HELPER")"
eval "$_plugin_fn"
_plugin_fn="$(sed -n '/^_workshop_size_human() {/,/^}/p' "$HELPER")"
eval "$_plugin_fn"
_plugin_fn="$(sed -n '/^_parse_workshop_vdf() {/,/^}/p' "$HELPER")"
eval "$_plugin_fn"
...
if echo "$_load" | grep -q "LOADED.*ACRE2"; then
...
# ===========================================================================
echo "── 18. Profile sync (syncprofiles) ──"
# ===========================================================================
# syncprofiles merges the prefix-local and host profile trees in both
# directions. It must never delete files (--delete is not used), because
# each side can hold profiles the other lacks.

_plugin_fn="$(sed -n '/^_sync_profiles() {/,/^}/p' "$HELPER")"
eval "$_plugin_fn"

MOCK_HOME="$TMPDIR_TEST/sync-home"
MOCK_COMPAT="$TMPDIR_TEST/sync-compat/107410"
export COMPAT_DATA_PATH="$MOCK_COMPAT"
export HOME="$MOCK_HOME"

# Build a prefix tree with a profile the host lacks.
mkdir -p "$MOCK_COMPAT/pfx/drive_c/users/steamuser/Documents/Arma 3 - Other Profiles/PVT"
mkdir -p "$MOCK_COMPAT/pfx/drive_c/users/steamuser/Documents/Arma 3"
touch "$MOCK_COMPAT/pfx/drive_c/users/steamuser/Documents/Arma 3 - Other Profiles/PVT/PVT.Arma3Profile"
touch "$MOCK_COMPAT/pfx/drive_c/users/steamuser/Documents/Arma 3/steamuser.Arma3Profile"

# Build a host tree with a profile the prefix lacks, plus a shared one
# whose host copy is NEWER (must not be overwritten by an older prefix copy).
mkdir -p "$MOCK_HOME/Documents/Arma 3"
mkdir -p "$MOCK_HOME/Documents/Arma 3 - Other Profiles/Matt"
mkdir -p "$MOCK_HOME/Documents/Arma 3 - Other Profiles/Shared"
touch "$MOCK_HOME/Documents/Arma 3 - Other Profiles/Matt/Matt.Arma3Profile"
touch "$MOCK_HOME/Documents/Arma 3/matt.Arma3Profile"
touch "$MOCK_HOME/Documents/Arma 3 - Other Profiles/Shared/Shared.Arma3Profile"
